...
router.post("/checkout", async (req, res) => {
    try {
        ...
        if (pedido.status !== StatusPedido.AGUARDANDO_PAGAMENTO) {
            throw new AppError("Pagamento só pode ser realizado para pedidos aguardando pagamento.", 400);
        }
...
router.post("/webhook", async (req, res) => {
    try {
        ...
        const pedidoAtualizado = await PedidoService.atualizarStatus(pedido.id, StatusPedido.RECEBIDO);
        try {
